...
Both vehicles' Turret profiles correctly show 0% Mobility — no vehicle's turret contains a mobility-critical component, and the classification pipeline reflects a real absence rather than defaulting to some nonzero placeholder (confirmed by `test_tiger_turret_never_produces_a_mobility_threshold`). Tiger's Hull profile shows a literal 0.0% Gun-classification chance: no gun/ammo zone exists in that profile's geometry at all. Sherman's Hull profile, by contrast, shows a real, non-zero Gun-classification chance (0.7% at this specific range/crew combination) driven directly by its sponson-mounted ammunition zones. While the quantitative gap at this range/crew is modest, the qualitative contrast is what validates the Sherman pilot choice: a genuine, present-but-modest, non-zero risk of hitting gun-critical components that simply does not exist in Tiger's hull geometry.

**A caveat found during the final whole-branch review**: at the specific 500m/Regular row above, the modeled 0.7% falls below the 1d6+1d8+1d12 dice sum's discrete resolution — `neither_below` and `mobility_at_or_above` both round to 21 at that row, so the Gun band is not actually rollable there (the same resolution floor the Gunnery Table already has elsewhere in this system, not something new to this mechanic). The gun outcome IS reachable in real play on 9 of Sherman Hull's 40 rows — for example, Militia crew at 100–750m (`gun_pct` 1.9%, `neither_below=21`/`mobility_at_or_above=22`, a genuine one-value-wide Gun band), and the 1500m row for every crew quality. This is the concrete result the Sherman pilot vehicle was chosen to test for: a real, sourced, sometimes-rollable Gun-zone risk that Tiger's hull geometry cannot produce at any row, range, or crew quality.
