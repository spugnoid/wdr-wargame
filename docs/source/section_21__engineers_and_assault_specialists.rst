...
.. list-table::
   ...
   * - F#
     - F1
     - Standard fire capability — engineers are an ordinary (F1) unit
   * - G#
     - G5
...
*Design note: real WWII infantry dug their own foxholes constantly, without waiting for engineer support. This rule extends Rule 21.7's own procedure to any combat unit, capped below what specialist tools and training buy an engineer.*
...
*Japan is deliberately absent from this table, and not as an oversight. Imperial Japanese Army combat engineers had no genuine squad-level, infantry-adjacent unit comparable to the German, Soviet, British and US entries above: engineers were concentrated in a division-level engineer regiment, sub-allotted by whole company (~250 men) to a supporting infantry regiment. Below platoon (~50 men), no sourced manpower or equipment breakdown exists at all. Printing a squad-scale "Japanese Engineers" row here, at the same stat resolution as the other four nations, would misrepresent a real organisational difference as a missing data point. A scenario needing IJA combat-engineer capability at the squad scale this table otherwise uses should stat a Rifle Squad (Rule 3.3.3's Japan example) with the DEMO capability added — small demolition charges and bangalore torpedoes are well documented as IJA engineer equipment — but without this table's building-assault bonuses (Rule 21.6.2), which represent a specific squad-level bunker-clearance doctrine no source documents for Japan at that scale.*
