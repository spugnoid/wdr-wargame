...
*Known gap: Rule 3.5.1 caps a hex at 3 combat units, with no exception for a unit returning from the Casualty Track (Rule 3.5.2 exempts only leaders). A Rally Point hex already at that limit when a Dispersed unit's rally roll succeeds has no defined resolution — the "always valid" wording in Rule 12.6a.3 was locked without reconciling it against the stacking limit. Deliberately left unresolved rather than patched in without real consideration; flagged here for whoever plays this out first.*
